      ...
      <div class="h-full" v-if="projectDetail">
        ...
        >
          ...
          <a-tab-pane key="download" tab="Download">
            <a-button class="mr-4" type="primary" v-if="projectDetail?.project_analysis_meta?.[0]?.h5ad_id"
                      @click="handleDownloadFile(projectDetail.project_analysis_meta[0].h5ad_id)">
              <template #icon>
                <CloudDownloadOutlined/>
              </template>
              H5AD File
            </a-button>
            <a-button class="mr-4" type="primary" v-if="projectDetail?.project_analysis_meta?.[0]?.umap_id"
                      @click="handleDownloadFile(projectDetail.project_analysis_meta[0].umap_id)">
              <template #icon>
                <CloudDownloadOutlined/>
              </template>
              Umap File
            </a-button>
            <a-button class="mr-4" type="primary" v-if="projectDetail?.project_analysis_meta?.[0]?.cell_marker_id"
                      @click="handleDownloadFile(projectDetail.project_analysis_meta[0].cell_marker_id)">
              <template #icon>
                <CloudDownloadOutlined/>
              </template>
              Cell Marker File
